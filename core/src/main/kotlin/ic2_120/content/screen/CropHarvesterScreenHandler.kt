...
@ModScreenHandler(block = CropHarvesterBlock::class, inventorySize = CropHarvesterBlockEntity.INVENTORY_SIZE)
class CropHarvesterScreenHandler(
    ...
    init {
        ...
        for ((i, slotIndex) in CropHarvesterBlockEntity.SLOT_CONTENT_INDICES.withIndex()) {
            addTrackedSlot(blockInventory, slotIndex, 48 + (i % 5) * 18, 17 + (i / 5) * 18)
        }
        for ((i, slotIndex) in CropHarvesterBlockEntity.SLOT_UPGRADE_INDICES.withIndex()) {
            addTrackedSlot(blockInventory, slotIndex, 152, 8 + i * 18)
        }
        addTrackedSlot(blockInventory, CropHarvesterBlockEntity.SLOT_DISCHARGING, 16, 53)
