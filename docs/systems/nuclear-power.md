...
- **热模式**：燃料棒产热（MOX 堆温 > 50% 时翻倍）→ 堆温管理 → 散热片加热冷却液 → 热能输出 HU
...
| > 8,500 | 5×5×5 | 方块有几率变成岩浆（仅原版方块：IC2 全系、其他模组方块、原版红石系列方块均不参与） |
| ≥ 10,000 | — | **核反应堆爆炸** |
...
| 组件 | 热容量 | 与组件交换 | 与堆交换 | 说明 |
|------|--------|-----------|---------|------|
| 反应堆冷却单元 (reactor_coolant_cell) | 10,000 | ✅ | ❌ | 被动热缓冲 |
| 三联反应堆冷却单元 (triple_reactor_coolant_cell) | 30,000 | ✅ | ❌ | 被动热缓冲 |
| 六联反应堆冷却单元 (sextuple_reactor_coolant_cell) | 60,000 | ✅ | ❌ | 被动热缓冲 |

- **热容量**：超过后组件损坏（消失）
- **与组件交换**：可与热交换器、元件散热片等组件双向交换热量
- **与堆交换**：不主动从反应堆吸热或向反应堆放热
- **用途**：作为被动热缓冲，吸收燃料棒产生的热量，或配合其他组件调节热量分布
...
| 组件 | 热量加成 | 爆炸修正 | 说明 |
|------|---------|---------|------|
| 反应堆隔板 (reactor_plating) | +500 | × 0.9 | 降低 10% 爆炸范围（累乘） |
| 高热容反应堆隔板 (reactor_heat_plating) | +1,700 | × 0.99 | 降低 1% 爆炸范围（累乘） |

- **热量加成**：永久增加反应堆热量上限
- **爆炸修正**：累乘计算，多个隔板效果相乘（例如：2 个反应堆隔板 = 0.9 × 0.9 = 0.81）
- **无热容量**：隔板本身不存储热量，不会损坏
...
| 组件 | 热容量 | 红石修复 | 青金石修复 | 说明 |
|------|--------|---------|-----------|------|
| 红石冷凝模块 (rsh_condensator) | 20,000 | ✅ 10k | ❌ | 只能用红石修复 |
| 青金石冷凝模块 (lzh_condensator) | 100,000 | ✅ 10k | ✅ 40k | 可用红石或青金石修复 |

- **热容量**：超过后组件损坏（消失）
- **热量单向流动**：只接受热量增加，**不允许热量减少**（不可被热交换器修复）
- **耐久不可逆**：热量增加会减少耐久，且**不能被反应堆内其他元件修复**
- **与冷却单元的区别**：
  - **冷却单元**：允许与其他组件双向热交换（可被热交换器吸热/放热）
  - **冷凝器**：只允许热量增加，不允许热量减少（耐久单向消耗）
- **工作台修复**：右键冷凝器 + 修复材料（红石/青金石）来恢复耐久
- **修复机制**：与工具修复类似，在物品栏中右键修复材料即可
